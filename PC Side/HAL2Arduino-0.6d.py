...
#                    print "commandHandler: checkSum passed"
                    if int(dataSet[0]) >= 800 and int(dataSet[0]) <= 899: #Client to Client traffic.
                        ...
                        masterTx.put(msg)
                    elif int(dataSet[0]) > 100 and int(dataSet[0]) < 550: # Inbound data for the Host.
                        #Process it.
...
#                        print "commandHandler: Got %d %d %d %d" % (command, axis, val, buildSum)
                        ...
                        if command == 503:
                            c['TemperatureBedIn'] = val

                        if command == 504:
                            c['TemperatureHeadIn'] = val

                        if command == 507:
                            c['TemperatureHeadReadyPin'] = val
...
# ## outputs here
        ...
        if codesAccepted.find("501") > -1:
            command=501
            hostCheck=c['TemperatureBedOut']
            if hostCheck != hostCheckOld[command]:
                hostCheckOld[command]=hostCheck
                checkSum=hostCheck+command
                comString=str(command) + " 0 " + str(hostCheck) + ";" #str(int(checkSum)) + ";"
        ...
        if codesAccepted.find("502") > -1:
            command=502
            hostCheck=c['TemperatureHeadOut']
            if hostCheck != hostCheckOld[command]:
                hostCheckOld[command]=hostCheck
                checkSum=hostCheck+command
                comString=str(command) + " 0 " + str(hostCheck) + ";" #str(int(checkSum)) + ";"
                masterTx.put(comString)
        if codesAccepted.find("505") > -1:
            command=505
            hostCheck=c['FanPin']
            if hostCheck != hostCheckOld[command]:
                hostCheckOld[command]=hostCheck
...
def comService():
    ...
    while threadsRun:
        ...
        try:
			...
			elif isinstance(dataSet[i], basestring):
			    print("Testif")
			    if dataSet[i] == "True":
				dataSet[i] = 1
			    if dataSet[i] == "False":
...
#Here we make our HAL pins (without duplicates.)
    ...
    if codesAccepted.find("501") > -1:
        if simulation == True:
            print "creating: TemperatureBedOut"
        else:
            print "makePins: creating: %r"
            c.newpin("TemperatureBedOut",hal.HAL_FLOAT,hal.HAL_IN)
    if codesAccepted.find("502") > -1:
        if simulation == True:
            print "creating: TemperatureHeadOut"
        else:
            print "makePins: creating: %r"
            c.newpin("TemperatureHeadOut",hal.HAL_FLOAT,hal.HAL_IN)
    if codesAccepted.find("503") > -1:
        if simulation == True:
            print "creating: TemperatureBedIn"
        else:
            print "makePins: creating: %r"
            c.newpin("TemperatureBedIn",hal.HAL_FLOAT,hal.HAL_OUT)
    if codesAccepted.find("504") > -1:
        if simulation == True:
            print "creating: TemperatureHeadIn"
        else:
            print "makePins: creating: %r"
            c.newpin("TemperatureHeadIn",hal.HAL_FLOAT,hal.HAL_OUT)
    if codesAccepted.find("505") > -1:
        if simulation == True:
            print "creating: FanPin"
        else:
            print "makePins: creating: %r"
            c.newpin("FanPin",hal.HAL_BIT,hal.HAL_IN)
    if codesAccepted.find("507") > -1:
        if simulation == True:
            print "creating: TemperatureHeadReadyPin"
        else:
            print "makePins: creating: %r"
            c.newpin("TemperatureHeadReadyPin",hal.HAL_BIT,hal.HAL_OUT)
    if codesAccepted.find("508") > -1:
        if simulation == True:
            print "creating: TemperatureBedReadyPin"
        else:
            print "makePins: creating: %r"
